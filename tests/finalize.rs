...
fn finalize_unauthorized_access() {
    ...
    let info = helpers::mock_info("creator1", &[funds]);
...
fn finalize_timing_validation() {
    ...
    let info = helpers::mock_info("creator", &[]); // Creator is the treasury
    let res = execute_finalize_stream(deps.as_mut(), env, info, 1, None);
...
fn finalize_happy_path() {
    ...
    assert_eq!(
        ...
        helpers::mock_info("creator", &[]).sender.to_string()
    );
    ...
    assert_eq!(
        ...
        &CosmosMsg::Bank(BankMsg::Send {
            to_address: helpers::mock_info("creator", &[]).sender.to_string(),
    ...
    assert_eq!(
        msg2,
        &CosmosMsg::Bank(BankMsg::Send {
            to_address: helpers::mock_info("collector", &[]).sender.to_string(),
...
fn finalize_duplicate_prevention() {
    ...
    let env = helpers::env_at(5_000_000 + 1); // After stream ends
    let info = helpers::mock_info("creator", &[]); // Creator is the treasury
    let res = execute_finalize_stream(deps.as_mut(), env.clone(), info.clone(), 1, None).unwrap();
